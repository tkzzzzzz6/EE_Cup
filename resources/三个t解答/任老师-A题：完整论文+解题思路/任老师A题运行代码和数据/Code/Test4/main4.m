...
%% 4. 构建序列输入 (使用 LMD 特征)
win = 96;
featureNames = {'lmd_totalirrad','lmd_diffuseirrad','lmd_temperature','lmd_pressure','lmd_winddirection','lmd_windspeed'};
F = T{:,featureNames};
P = T.power_clipped;
t = T.date_time;
...
for i = win+1:length(P)
    if all(~isnan(P(i-win:i))) && all(all(~isnan(F(i-win:i,:))))
        seq = [P(i-win:i-1), F(i-win+1:i,:)];  % 功率 + LMD 滑窗
        X{end+1} = seq';
        Y(end+1,1) = P(i);
    end
end
...
Pcs = T.P_cs(win+1:end); Pcs = Pcs(is_test);
is_day = Pcs > 0;

EMAE = mean(abs(YPred(is_day) - YTest(is_day))) / P_rated;
ERMSE = sqrt(mean((YPred(is_day) - YTest(is_day)).^2)) / P_rated;
fprintf('EMAE = %.4f, ERMSE = %.4f\n', EMAE, ERMSE);

%% 9. 绘图
figure;
plot(YTest,'k'); hold on;
plot(YPred,'r--'); legend('实际','预测');
xlabel('样本'); ylabel('功率(kW)'); title('LSTM + LMD'); grid on;
...
writetable(result_table, 'lstm_prediction_result_with_lmd.csv');
fprintf('预测结果已保存至 lstm_prediction_result_with_lmd.csv\n');
